...
function! s:define_actions() abort
  ...
  call gita#action#include([
        ...
        \], g:gita#ui#ls_files#disable_default_mappings)
  if g:gita#ui#ls_files#disable_default_mappings
    return
  endif
  execute printf(
        \ 'nmap <buffer> <Return> %s',
        \ g:gita#ui#ls_files#primary_action_mapping
        \)
endfunction
...
function! s:on_BufReadCmd(options) abort
  ...
  call gita#ui#ls_files#redraw()
  call gita#util#select(options.selection)
endfunction


function! gita#ui#ls_files#autocmd(name) abort
  let options = gita#util#cascade#get('ls-files')
  call call('s:on_' . a:name, [options])
endfunction

function! gita#ui#ls_files#open(...) abort
  let options = extend({
        \ 'anchor': 0,
        \ 'opener': '',
        \}, get(a:000, 0, {}))
  ...
  let opener = empty(options.opener)
        \ ? g:gita#ui#ls_files#default_opener
        \ : options.opener
...
function! gita#ui#ls_files#redraw() abort
  let git = gita#core#get_or_fail()
...
call gita#util#define_variables('ui#ls_files', {
