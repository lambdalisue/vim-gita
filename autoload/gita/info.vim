...
function! s:get_info(...) abort " {{{
  ...
  let status = gita.git.get_parsed_status(extend({
        \ 'ignore_submodules': 1,
        \}, get(g:, 'gita#info#status_options', {}),
        \))
  let status_count = {
        \ 'conflicted': 0,
        \ 'unstaged': 0,
...
" Public functions
function! gita#info#get(...) abort " {{{
  return call('s:get_info', a:000)
endfunction " }}}
function! gita#info#format(format, ...) abort " {{{
  let expr = get(a:000, 0, '%')
  ...
  elseif s:P.is_dict(expr)
    ...
    throw 'vim-gita: a second argument of gita#info#format must be a string or dictionary.'
  endif
  return s:format_string(a:format, info)
endfunction " }}}
function! gita#info#preset(preset_name, ...) abort " {{{
  let expr = get(a:000, 0, '%')
  if s:P.is_string(expr)
    let info = s:get_info(expr)
  elseif s:P.is_dict(expr)
...
function! gita#info#clear(...) abort " {{{
