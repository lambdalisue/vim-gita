...
function! gita#features#commit#open(...) abort " {{{
  ...
  " Define hooks
  function! b:_gita_hooks.ac_BufWinLeave_pre(bufnum) abort
    if !getbufvar(a:bufnum, '&modified') && gita#utils#asktf('Do you want to commit changes?', 'y')
      call s:commit(a:bufnum, {})
    endif
  endfunction
