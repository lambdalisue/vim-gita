...
function! s:ensure_revision(options, default) abort " {{{
  if !has_key(a:options, 'revision') || get(a:options, 'ask_revision')
...
let s:parser = s:A.new({
      \ 'name': 'Gita open',
      \ 'description': [
      \   'Open a file content of a specified revision.',
      \ ],
      \})
...
call s:parser.add_argument(
      \ 'revision', [
      \   'A commit-ish revison (e.g. HEAD@{4}) which you want to check.',
      \ ], {
...
function! gita#features#open#show(...) abort " {{{
...
function! gita#features#open#command(bang, range, ...) abort " {{{
  let options = s:parser.parse(a:bang, a:range, get(a:000, 0, ''))
  if !empty(options)
    call gita#features#open#show(options)
  endif
endfunction " }}}
function! gita#features#open#complete(arglead, cmdline, cursorpos) abort " {{{
