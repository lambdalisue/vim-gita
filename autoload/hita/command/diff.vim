...
let s:MAPPING_TABLE = {
      ...
      \ '<Plug>(hita-diff-vartical)': 'Diff an (INDEX) content in two window (vertical)',
...
function! hita#command#diff#action(candidates, ...) abort
  let options = extend({
        \ 'opener': '',
        \ 'split': '',
        \ 'anchor': 1,
...
function! hita#command#diff#define_plugin_mappings() abort
  noremap <buffer><silent> <Plug>(hita-diff)
        \ :call hita#action#call('diff')<CR>
  noremap <buffer><silent> <Plug>(hita-diff-above)
        \ :call hita#action#call('diff', {'opener': 'leftabove new'})<CR>
  noremap <buffer><silent> <Plug>(hita-diff-below)
        \ :call hita#action#call('diff', {'opener': 'rightbelow new'})<CR>
  noremap <buffer><silent> <Plug>(hita-diff-left)
        \ :call hita#action#call('diff', {'opener': 'leftabove vnew'})<CR>
  noremap <buffer><silent> <Plug>(hita-diff-right)
        \ :call hita#action#call('diff', {'opener': 'rightbelow vnew'})<CR>
  noremap <buffer><silent> <Plug>(hita-diff-tabnew)
        \ :call hita#action#call('diff', {'opener': 'tabnew', 'anchor': 0})<CR>
  noremap <buffer><silent> <Plug>(hita-diff-pedit)
        \ :call hita#action#call('diff', {'opener': 'pedit', 'anchor': 0})<CR>
  noremap <buffer><silent> <Plug>(hita-diff-vertical)
        \ :call hita#action#call('diff', {'opener': 'edit', 'split': 'vertical'})<CR>
  noremap <buffer><silent> <Plug>(hita-diff-horizontal)
        \ :call hita#action#call('diff', {'opener': 'edit', 'split': 'horizontal'})<CR>
endfunction
function! hita#command#diff#define_default_mappings() abort
...
function! hita#command#diff#get_mapping_table() abort
